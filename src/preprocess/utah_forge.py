...
def build_utah_forge_state(
    ...
    friction_col = mapping.get("friction_coefficient")

    if time_col is None or tau_col is None:
        raise ValueError("Utah FORGE preprocessing requires mapped time and tau columns.")
    if velocity_col is None and displacement_col is None:
        raise ValueError("Utah FORGE preprocessing requires either a velocity column or a displacement column.")
    ...
    metadata["preserved_columns"] = [column for column in working.columns if column not in {"time", "tau", "V"}]
